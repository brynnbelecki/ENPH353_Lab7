...
class Gazebo_Linefollow_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        _, threshold = cv2.threshold(gray, 147, 255, cv2.THRESH_BINARY)
        ...
        for bin in range(10):
            ...
            for j in range(int(width / 10)):
                if threshold[height - 5][int(width / 10) * bin + j] == 0:
                    count += 1
                    if count > max_count:
                        max_count = count 
                        line_loc = bin
        ...
        color = (0, 0, 0) 
        thickness = 1
        cv2.putText(cv_image, text, org, font, fontScale, color, thickness, cv2.LINE_AA)
    ...
    def step(self, action):
        ...
        driving = False
        # check if went forward in last 20 images
        # if len(self.episode_history) > 20:
        #     for i in range(20):
        #         if self.episode_history[-1*i] == 0:
        #             driving = True
        # else:
        #     driving = True
        ...
        if not done:
            if action == 0:  # FORWARD
                reward = 4
            # elif not driving: #penalize for going forward too infrequently
            #     reward = -10
            # elif action == 1:  # LEFT
            #     reward = 2
            # else:
