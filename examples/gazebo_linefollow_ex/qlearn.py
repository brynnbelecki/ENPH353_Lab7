...
class QLearn:
    ...
    def chooseAction(self, state, return_q=False):
        ...
        if random.random() < self.epsilon: 
            ...
            for action in self.actions:
                ...
                elif self.getQ(state, action) == maxQ and random.randint(0,1) == 0: #update Q value half the time if equal for two actions 
                    bestAction = action
    ...
    def learn(self, state1, action1, reward, state2):
        ...
        #       is not in our dictionary? This is covered in implementation of 
        #       getQ (it returns 0, and is updated according to the next state)
        ...
        Qs1a1 = self.getQ(state1, action1)

        maxQs2 = 0
        for action in self.actions:
            if self.getQ(state2, action) > maxQs2:
                maxQs2 = self.getQ(state2, action) 

        Qnew = Qs1a1 + self.alpha * (reward + self.gamma * maxQs2 - Qs1a1)
        
        self.q[(state1,action1)] = Qnew
